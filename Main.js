...
Canvas.registerFont("./Texture/CJKkr.ttf",{family:"Kr"})
...
client.on("message", async (message)=>{
    ...
    switch (command) {
        case "help":
            ...
            embed.addField(prefix+"aud","메시지를 보낸 서버의 인원을 확인 가능합니다.");
            embed.addField(prefix+"invite","메시지를 보낸 서버의 초대 링크를 가져옵니다.");
            embed.addField(prefix+"rule","메시지를 보낸 서버의 규칙을 확인합니다.");
        ...
        case "aud":
            ...
            embed.setDescription(`현재 ${message.guild.memberCount}명의 유저가 있습니다!`)
            message.channel.send(embed)
            break;
        case "invite":
            embed.setAuthor("초대 코드",client.user.avatarURL());
            embed.setDescription("이 서버의 초대코드는 https://discord.gg/hNnBvAn 입니다.")
            message.channel.send(embed);
            break;
        case "rule":
        ...
        case "ee":
            if(arg[0].length <= 11){
                ...
                json = await got(`https://www.googleapis.com/youtube/v3/videos?key=${Setting.Youtube_Token}&part=status&id=${arg[0]}`);
                body = JSON.parse(json.body);
                if(body.pageInfo.totalResults == 0){
                    embed.setDescription("영상 검색 결과가 없습니다.")
                         .setColor("RED")
                    message.channel.send(embed);
                } else {
                    if(body.items[0].status.privacyStatus == "unlisted"){
                        embed.setDescription(`올바른 유튜브 링크로 이어집니다!\n\n[보러가기](https://www.youtube.com/watch?v=${body.items[0].id})`)
                             .setImage(`https://i.ytimg.com/vi/${body.items[0].id}/maxresdefault.jpg`)
                             .setColor("GREEN")
                        message.channel.send(embed);
                    } else {
                        embed.setDescription("비공개 영상이 아닙니다.")
                             .setColor("RED")
                        message.channel.send(embed);
                    }
                }
...
client.on("guildMemberAdd", async(member)=>{
    welcome = await guild.channels.cache.find(channel => channel.id == "695993355339038721");
    ...
    avatar = await Canvas.loadImage(member.user.displayAvatarURL({format: 'png'}));

    ctx.beginPath();
    ctx.arc(canvas.width/2,canvas.height/3,256/2+5,0,Math.PI * 2, true);
    ctx.closePath();
    ctx.clip();
    ctx.fillRect(0,0,1100,500);
    ctx.beginPath();
    ...
    ctx.closePath();
    ctx.clip();
    ctx.drawImage(avatar,canvas.width/2-128,canvas.height/3-128,256,256)
    ...
    await File.readFile("./Data/Roles.json","utf-8", async (err,data)=>{
        if(err){
            ...
            if(Roles.find(o=> o.id == member.id)){
                await Roles.find(o=> o.id == member.id).roles.forEach( async (role) =>{
                   member.roles.add(await guild.roles.fetch(role),"원래 있던 역할").catch((err)=>{
                       err_channel.send(err);
                   })
                })
            }
...
client.on('messageReactionAdd',async (msr,user) => {
    ...
    if(msr.message.channel.topic == user.id){
        ...
        ManageMessage = ManageMessages.find(message => message.embeds[0].fields.find(field => field.name == "ID").value == user.id)
    ...
    if(msr.message.channel.id == manage.id && !(user.bot)){
        ...
        ReportChannel = client.channels.cache.find(channel => msr.message.embeds[0].fields[0].value == channel.topic);
        ...
        await ReportChannelMessages.sort((userA, userB) => userA.createdTimestamp - userB.createdTimestamp);
        await ReportChannelMessages.forEach(message => {
            if(message.content){
                LogEmbed.addField(guild.member(message.author).displayName +" | "+ moment(message.createdTimestamp).tz("Asia/Seoul").format('YYYY-MM-DD h:mm:ss'),message.content);
            }
        });
    ...
    if((msr.message.id == '719182514270240768') && !(user.bot)){ 
        ...
        embed.setDescription("신고 채널이 생성 되었습니다!");
        embed.setFooter(`해당 신고방을 닫고 로그를 남기려면 밑 이모지를 눌러 주시길 바랍니다.`)
    ...
    await File.readFile("./Data/Roles.json","utf-8" , async (err,data) => {
        if(err){
            ...
            if(map.find(o => o.id == member.id)){
                map.splice(map.findIndex(o => o.id == member.id),1);
            }
            map.push({
                id : member.id,
                roles : roles
            });
            await File.writeFile("./Data/Roles.json",JSON.stringify(map),async (err)=>{
                if(err) {
                    err_channel.send(err);
                } 
            });
